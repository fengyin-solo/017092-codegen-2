...
| 模块         | 说明 |
|--------------|------|
| 概览         | 知识总量与四类知识柱状图/饼图；提供「知识分布分析」入口 |
| 知识分布分析 | 从概览进入，可在「按商家 / 按行业」间切换，展示每个对象的知识数量与覆盖状态；数量为 0 的商家/行业同样展示，支持「仅看未覆盖」并一键跳转补充知识 |
...
└── frontend-admin/      # 运营端中后台
    ├── login.html       # 登录页面
    ├── index.html       # 首页/概览
    ├── distribution.html # 知识分布分析（按商家/行业）
